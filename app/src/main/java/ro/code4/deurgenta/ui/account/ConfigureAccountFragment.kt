...
import ro.code4.deurgenta.ui.address.ConfigureAddressFragmentDirections
...
class ConfigureAccountFragment : ViewModelFragment<ConfigureAccountViewModel>() {
    ...
    private fun configureCallbacks() {
        viewBinding.callbackAddresses = object : ViewCallback {
            override fun call() {
                val bundle = Bundle()
                //val directions = ConfigureAccountFragmentDirections.actionConfigureAddress()
                findNavController().navigate(R.id.action_configure_address)
            }
        }
    }
}